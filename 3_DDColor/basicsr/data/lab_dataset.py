...
class LabDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.fmix_params = {'alpha':1.,'decay_power':3.,'shape':(opt['gt_size'],opt['gt_size']),'max_soft':0.0,'reformulate':False}
    ...
    def __getitem__(self, index):
        ...
        if self.do_fmix and np.random.uniform(0., 1., size=1)[0] > self.fmix_p:
            with torch.no_grad():
                ...
                fmix_index = random.randint(0, len(self.paths) - 1)
        ...
        if self.do_cutmix and np.random.uniform(0., 1., size=1)[0] > self.cutmix_p:
            with torch.no_grad():
                cmix_index = random.randint(0, len(self.paths) - 1)
